...
build-darwin: fmt
	...
	@mkdir -p bin
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o bin/${BINARY} main/service.go
...
build: fmt
	...
	@mkdir -p bin
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o bin/${BINARY} main/service.go
	@echo "$(CGREEN)Build Success!$(CEND)"
	@echo "$(CGREEN)Begin packing ...$(CEND)"
	@rm -rf dist/*
	@mkdir -p dist/{bin,logs}
	@cp bin/${BINARY} dist/bin/
	@cp config_prod.json dist/config.json
	@cp config_kolradar.json dist/config_kolradar.json
	@cp run.sh dist/
	#@tar zcvf dist.tgz dist
	@echo "$(CGREEN)End packing ...$(CEND)"
